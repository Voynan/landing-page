...
- **Audience and job:** Designers and developers must navigate nine named specimens, compare tokens with live production components, and discover accessibility or responsive defects without consulting private implementation state.
...
- **Constraints:** One `h1`; nine `section` regions; no public navigation, prerender, canonical, hreflang, sitemap, or social metadata; `noindex,nofollow`; real production primitives only. Media, Motion, and Forms document their production contracts with explicit resilient states and deterministic internal adapters; no specimen calls a production endpoint or invents public product facts.
